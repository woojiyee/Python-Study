...
# 좌표를 넣을거니까 []를 넣자.
# 선언할 때 리스트 []나 튜플() 없이 그냥 queue = deque()만 선언해주고
# 큐에 추가할 때 원하는 자료형으로 넣어줘도 됨
queue = deque([])
# 방향 리스트. [dx[0], dy[0]]은 곧 [-1, 0]이고 이는 왼쪽으로 이동하는 위치
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0 <= nx < n and 0 <= ny < m and matrix[nx][ny] == 0:
                # 큐여 넣음으로써 익은 토마토가 된 것을 의미 
                # -> 왜냐하면 또 큐에 들어가있는 주위 토마토들을 체킹하는 셈이니까
                # 문제에서 토마토가 익어있다면 주위 토마토들도 익는다 <-> 큐에 들어간 노드는 주위 노드가 조건에 맞을시(범위 안, 0인 노드) 체킹한다.
                queue.append([nx, ny])
                # 익히고 1을 더해주면서 횟수를 세어주기
                # 여기서 나온 제일 큰 값이 정답이 될 것임
                matrix[nx][ny] = matrix[x][y] + 1
                # 원래 0 이었던 matrix[nx][ny]에 값을 넣어줌으로써 일자도 세고 + 0이 아닌 값이 있다면 이미 방문했음을 의미
                # -> 해당 if문에 matrix[nx][ny] == 0 은 재방문하지도 않았고 + -1이 아닌 안 익은 토마토 0이라는 조건 두개가 검사 됨
...
for i in matrix:
    for j in i:
        ...
        if j == 0:
            print(-1)
            # 코드를 강제 종료하고 싶을 때 사용, 프로그램 자체를 종료
            exit(0)
    # 다 익혔다면 최댓값이 정답
    # max(순회가능한 자료형1, 자료형2) = (1 중에 최대값, 2 중에 최대값) 중에 최대값을 반환
    # 2차원 배열이라 for문 돌려서 1차원씩 최대 요소를 구함
    # -> 각 1차원 배열 중 최댓값 n 개 중 최대값을 또 구해야 함
    # 그래서 res에 전 행의 최댓값을 넣게 해서 현재 행의 최댓값과 비교하여 또 현재행까지의 최대값을 넣게 함
    # res = max(res, max(i)) 이렇게 짬으로써 res는 기록?되는 기능을 하는 듯
    res = max(res, max(i))
    # i = 0 res = max(0,max(0)) = max(0)
    # i = 1 res = max(res,max(1)) = max(max(0),max(1)) -> 전 행과 현재 행을 비교하여 최대값을 저장
# 처음 시작을 1로 표현했으니 1을 빼준다.
# + 이미 다 익어있는 경우 matrix의 최대값은 1일 거임 저장될 때 익은 토마토가 1이고 bfs 돌려도 bfs로 인한 변화는 없겠지 0인 토마토가 없으니까
#   다 익은 경우 res = max(0,1) = 1인데 아래 res -1함으로써 다 익은 경우도 0이 출력됨...
print(res - 1)
